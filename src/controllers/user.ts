...
export const userFollowing = async (req: Request, res: Response) => {
	...
	if (following && follower)
		return res.status(200).send({ msg: 'Following added succesfully', following, follower });
	return res.status(500).send({ msg: 'Internal server error' });
};
...
export const userFollowers = async (req: Request, res: Response) => {
	...
	if (resp) return res.status(200).send({ msg: 'Followers added succesfully', resp });
	return res.status(500).send({ msg: 'Internal server error' });
};
...
export const userReport = async (req: Request, res: Response) => {
	const { idUser } = req.params;
	const { report } = req.body;
	console.log(idUser);
	console.log(report);
	const resp = await addReport(idUser, report);

	if (resp) return res.status(200).send({ msg: 'Report received', resp });
	return res.status(500).send({ msg: 'Internal server error' });
};
...
export const readNot = async (req: Request, res: Response) => {
	...
	try {
		...
		if (modResponse.modifiedCount == 1)
			return res.status(202).send({ response: true, payload: 'Updated successfully' });

		// Si no se modificó ningún usuario, entonces retorna un 400,
		// haciendo referencia a que el usuario no existe
		return res.status(400).send({ response: false, payload: 'Invalid provided info' });
	} catch (error) {
...
export const delNot = async (req: Request, res: Response) => {
	...
	try {
		// Query principal
		const modResponse = await User.updateOne({ _id: id }, { $set: { notifications: [] } });
		// Si se modificó con exito, entonces response con un 202
		if (modResponse.modifiedCount == 1)
			return res.status(202).send({ response: true, payload: 'Updated successfully' });
