    ...
    User.findOneAndUpdate(
      ...
      function (error: any, _success: any) {
        if (error) {
          return res
            .status(400)
            .send({ response: false, msg: 'Post was not published' });
        }
...
export const cuackPut = async (req: Request, res: Response) => {
  ...
  if (id && likeID && cuackID) {
    ...
    if (response) {
      return res
        .status(202)
        .send({ response: true, msg: 'Likes updated successfully' });
    } else {
      return res
        .status(500)
        .send({ response: false, msg: 'Error in like post' });
    }
  }
  ...
  if (id && comment && cuackID) {
    ...
    if (response) {
      return res
        .status(202)
        .send({ response: true, msg: 'Comment updated successfully' });
    } else {
      return res
        .status(500)
        .send({ response: false, msg: 'Error, you could comment' });
    }
  }
  ...
  if (id && recuackID && cuackID) {
    ...
    if (response) {
      return res
        .status(202)
        .send({ response: true, msg: 'Recuack updated successfully' });
    } else {
      return res
        .status(500)
        .send({ response: false, msg: 'Error in recuack post' });
    }
  }
